"use client"
import React from "react";
import { FaRegCheckSquare } from 'react-icons/fa';
import Typewriter from "typewriter-effect";
const Skill = () => {
  return (
    <div id="skills">
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-col text-center w-full mb-20">
            <h2 className="text-3xl text-gray-900 tracking-widest font-medium title-font mb-1">
            <Typewriter
                options={{
                  strings: ["My SKILLS"],
                  autoStart: true,
                  loop: true,
                }}
              />
            </h2>
            
          </div>
          <div className="flex flex-wrap -m-4 -mt-[5rem]">
            {/*  SKILLS */}
            <div className="p-4 w-[100%] md:w-1/3">
              <div className="flex rounded-lg h-full  p-8 flex-col">
                <div className="flex items-center mb-3">
                  <div className="w-8 h-8 mr-3 inline-flex items-center justify-center rounded-full bg-purple-500 text-white flex-shrink-0" >
                    <FaRegCheckSquare />
                  </div>
                  <h2 className="text-gray-900 text-lg title-font font-medium">
                    HTML
                  </h2>
                </div>
                <div className="flex-grow">
                  <div className="relative h-1 w-full bg-gray-300 rounded-xl">
                  <div  className="absolute bg-purple-500 h-1 rounded-xl w-[100%]"></div>
                  </div>
                  <p className="font-bold text-purple-500 text-right">100%</p>

                
              
             
                </div>
              </div>
            </div>
            {/*  SKILLS */}
            <div className="p-4 w-[100%] md:w-1/3">
              <div className="flex rounded-lg h-full  p-8 flex-col">
                <div className="flex items-center mb-3">
                  <div className="w-8 h-8 mr-3 inline-flex items-center justify-center rounded-full bg-purple-500 text-white flex-shrink-0" >
                    <FaRegCheckSquare />
                  </div>
                  <h2 className="text-gray-900 text-lg title-font font-medium">
                    CSS
                  </h2>
                </div>
                <div className="flex-grow">
                  <div className="relative h-1 w-full bg-gray-300 rounded-xl">
                  <div  className="absolute bg-purple-500 h-1 rounded-xl w-[70%]"></div>
                  </div>
                  <p className="font-bold text-purple-500 text-right">70%</p>
                </div>
              </div>
            </div>
            {/*  SKILLS */}
            <div className="p-4 w-[100%] md:w-1/3">
              <div className="flex rounded-lg h-full  p-8 flex-col">
                <div className="flex items-center mb-3">
                  <div className="w-8 h-8 mr-3 inline-flex items-center justify-center rounded-full bg-purple-500 text-white flex-shrink-0" >
                    <FaRegCheckSquare />
                  </div>
                  <h2 className="text-gray-900 text-lg title-font font-medium">
                    TypeScript
                  </h2>
                </div>
                <div className="flex-grow">
                  <div className="relative h-1 w-full bg-gray-300 rounded-xl">
                  <div  className="absolute bg-purple-500 h-1 rounded-xl w-[80%]"></div>
                  </div>
                  <p className="font-bold text-purple-500 text-right">80%</p>
                </div>
              </div>
            </div>
            {/*  SKILLS */}
            <div className="p-4 w-[100%] md:w-1/3">
              <div className="flex rounded-lg h-full  p-8 flex-col">
                <div className="flex items-center mb-3">
                  <div className="w-8 h-8 mr-3 inline-flex items-center justify-center rounded-full bg-purple-500 text-white flex-shrink-0" >
                    <FaRegCheckSquare />
                  </div>
                  <h2 className="text-gray-900 text-lg title-font font-medium">
                    NEXT.JS
                  </h2>
                </div>
                <div className="flex-grow">
                  <div className="relative h-1 w-full bg-gray-300 rounded-xl">
                  <div  className="absolute bg-purple-500 h-1 rounded-xl w-[60%]"></div>
                  </div>
                  <p className="font-bold text-purple-500 text-right">60%</p>
                </div>
              </div>
            </div>
           </div>
        </div>
      </section>
    </div>
  );
};

export default Skill;
